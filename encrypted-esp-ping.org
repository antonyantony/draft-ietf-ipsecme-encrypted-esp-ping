...
#+begin_abstract
This document defines an Encrypted ESP Echo Function. a mechanism
designed to assess the reachability of an IP Security (IPsec) network
path using Encapsulating Security Payload (ESP) packets. The primary
objective of this function is to facilitate the detection of
end-to-end path status dynamically in a reliable and efficient
manner, only using encrypted ESP packets between the IPsec peers. The
Encrypted Echo message can use existing congestion control payloads
from RFC9347 or the message format specified here, with an optional
...
Security Payload (ESP), this document introduces Encrypted ESP Ping;
Echo request and response. This protocol offers a solution for
assessing network path reachability and can optionally specify a
return path for echo reply messages.

This document covers only Encrypted ESP Ping, while
[[I-D.colitti-ipsecme-esp-ping]] specifies an unauthenticated ESP
Ping.
...
encapsulated in UDP, i.e using protocol ESP, successful IKE
...
due to broken packet filtering. Although this is less likely, it is
still possible and can be hard to diagnose. Operational experience
suggests that networks and some home routers that drop ESP packets
are common enough to be a problem for general-purpose VPN
applications desiring to work reliably on the Internet. Encrypted ESP
Ping would be a great help to diagnose these scenarios.
...
network paths and may have different policies. The ESP Encrypted Ping
...
attributes are necessary. The initiator may propose a specific SPI as
...
- Identifier : A 16-bit request identifier. The identifier might be set
...
Further this can be rate limited or filtered using ingress filtering
per BCP 38 [[RFC2827]]
